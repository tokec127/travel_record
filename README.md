...
- 여행 기록은 사진 썸네일과 부가정보를 2컬럼으로 표시하고, 종료 위치는 여행 종료일 다음 날부터 표시한다. `+` 아이콘으로 결제·메모 카드를 추가하며, 사진 기록을 선택해 메모 수정 또는 메타데이터 삭제를 수행한다. 원본 사진은 유지한다.
- 동일 그룹 사진 카드를 선택하면 그룹 사진을 96×96px 고정 썸네일 묶음으로 팝업에 표시하고 가용 폭에 따라 자동 줄바꿈한다.
